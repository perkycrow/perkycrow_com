...
class DefendTheDen extends Game {
  ...
  configureGame() {
    ...
    this.on("update", (delta) => {
      ...
      this.dayNightPass.setUniform("uTime", performance.now() / 1e3);
      if (this.dayPaused) {
        return;
      }
  ...
  onStart() {
    __privateMethod(this, _DefendTheDen_instances, buildTextureAtlases_fn).call(this);
...
showCommandPalette_fn = async function() {
  if (!__privateGet(this, _commandPaletteEl)) {
    const { default: DevToolsCommandPalette } = await __vitePreload(async () => {
      const { default: DevToolsCommandPalette2 } = await import("./devtools_command_palette-60alxmdM.js");
